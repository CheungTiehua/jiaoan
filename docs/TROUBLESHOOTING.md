...
**排查**：
- 未登录 / 登录过期：返回 401，需重新登录
- 教案内容过短：返回"教案内容过短，无法生成思维导图"，确保教案内容超过 100 字符
- API Key 未配置：检查 `.env` 或 `data/api_key.json`
- DeepSeek 限流：返回 429，等待 1 分钟后重试
- 生成质量不达标：自动重试一次，仍失败则提示重试
...
**排查**：
- 导图源码仍可复制，粘贴到 Mermaid 在线工具查看
- 点击"查看 Mermaid 源码"确认内容是否完整
- 可能是 Mermaid 语法兼容性问题，不影响使用

## 16. 历史记录没有保存导图

**现象**：打开历史记录，导图 Tab 显示"尚未生成思维导图"

**排查**：
- 确认之前生成导图时是否成功（如有错误提示）
- 老教案可能尚未生成过导图，点击"生成思维导图"重新生成
- 如果页面提示"导图已生成，但保存到历史记录失败"，检查后端服务是否正常

## 17. 导出文档没有导图

**现象**：下载的 MD/DOCX 不含导图附录

**排查**：
- 确认使用的是"下载 MD（含导图）""下载 DOCX（含导图）"按钮，而非普通导出按钮
- 确认当前教案已生成思维导图（检查导图 Tab 是否有内容）
- 只有已生成的导图才会出现在导出文档中
